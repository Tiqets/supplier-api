...
def products_printer(products: List[Product]) -> None:
    for p in products:
        ...
        if p.required_order_data:
            pdetail(
                'Required Additional Order Data', 
                ', '.join([x.value for x in p.required_order_data]),
            )
        if p.required_visitor_data:
            pdetail(
                'Required Additional Visitors Data',
                ', '.join([x.value for x in p.required_visitor_data]),
            )
        pdetail('Provides Pricing', p.provides_pricing)
        print('')
